...
var Standalone = require('mongodb-topology-manager').Server;
...
//     if (Number(opts.version) > 2.6) {
//       newOpts.replicasetOptions.setParameter = 'authenticationMechanisms=' + opts.auth_mechanism;
//     }
...
var startStandalone = function(opts) {
  var options = {
    port: opts.port,
    dbpath: opts.dbpath
    // ,logpath: opts.logpath // --> mongodb-topology-manager requires log to stdout
    // TODO: purge: opts.purge --> purge the dbpath?
  };
  return new Standalone('mongod', options);
};
...
var startReplicaset = function(opts) {
  ...
  return new ReplSet('mongod', [{
    // mongod process options
    options: {
      bind_ip: 'localhost', port: opts.port, dbpath: opts.dbpath + '/' + (opts.port)
    }
  }, {
    // mongod process options
    options: {
      bind_ip: 'localhost', port: opts.port + 1, dbpath: opts.dbpath + '/' + (opts.port + 1)
    }
  }, {
    // Type of node
    arbiterOnly: true,
    // mongod process options
    options: {
      bind_ip: 'localhost', port: opts.port + 2, dbpath: opts.dbpath + '/' + (opts.port + 2)
    }
  }], {
    replSet: opts.name
  });
...
module.exports = function(opts, done) {
  var server;
  var delay = 0;  // this is used for auth, currently unused
  ...
  debug('starting', JSON.stringify(opts, null, 2));
  /**
  ...
  /* NEW topo-manager */
  /* mongodb-topology-manager requires mongod to output to stdout (logpath cannot be set) */
  /* requires modifying topology-manager to resolve() with pid for this to work */
  if (opts.purge) {
    server.purge();
  }
  server.discover();
  ...
  //         createAdmin(db, opts.username, opts.password, function(err, db) {
  //           if (err) {
  //             return done(err);
  //           }
  ...
  //             } else {
  //               server.setCredentials('mongodb-cr', 'admin', opts.username, opts.password);
  //             }
