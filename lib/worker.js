...
module.exports = function(opts, done) {
  ...
  // if (opts.purge) {
  //   debug('Purging');
  //   server.purge().then(function() {
  //     debug('Purged');
  //     debug('Starting');
  //     server.start();
  //     if (process.send) {
  //       var msg = {event: 'started', opts: opts};
  ...
  //// this works, but eslint complains about function*()
  co(function*() {
    if (opts.purge) {
      debug('Purging................');
      server.purge();
    ...
    debug('Starting.................');
    // server.start(); // if yield is used here, it doesn't work
    ...
    // any code here is always executed
    if (process.send) {
      var msg = {event: 'started', opts: opts};
      debug('Notifying parent process', JSON.stringify(msg, null, 2));
      process.send(msg);
  ...
  //// this doesn't execute the code inside then()
  // server.start().then(function() {
  //   debug('Started');
